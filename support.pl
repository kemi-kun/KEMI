...
%! prepend_prefix(+String:string, +Prefix:string, -Result:string) is det.
%! prepend_prefix(+String:string, +Prefix:string, +Result:string) is semidet.
%
%  Prepend `Prefix` to the name of `Element`.
%
prepend_prefix(String, Prefix, Result) :-
    (
        prepend_prefix_alternative(String, Prefix, Result);
...
prepend_prefix_alternative("oxide", "mono", "monoxide").
